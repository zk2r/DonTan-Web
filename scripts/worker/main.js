...
function hiraganaToBrainfuck(program) {
    return program.replace(/どん/g, '>').replace(/たん/g, '<').replace(/どど/g, '+').replace(/うー/g, '-').replace(/あべばべばばば/g, '.').replace(/！/g, ',').replace(/はい/g, '[').replace(/わお/g, ']');
}
function compileBrainfuck(program, options) {
    return __awaiter(this, void 0, void 0, function* () {
        yield initPromise;
        const canon = canonicalOptions(options);
        const bfProgram = hiraganaToBrainfuck(program);
        return wasm_bindgen.compile_brainfuck(bfProgram, canon.numCells, canon.cellSize, quantifyEOF(canon.eof));
    });
}
